          ...
          <form @submit.prevent="handleAddShipmentSubmit" class="shipment-form-in-modal">
            <div class="form-grid">
              ...
              <div>
                ...
                <input type="text" id="modal-batch-number" v-model="newShipment.batch_number" required />
              </div>
              <div>
            ...
            <div class="production-slots-form-in-modal section-block-modal">
             ...
             <p v-if="addError" class="error-message modal-error">{{ addError }}</p>
      ...
      <table ref="actualTable">
        <thead>
          <tr>
            ...
            <template v-for="idx in max_production_slots" :key="'header-slot-' + idx">
              <th class="dynamic-slot-time-column">起止时间 {{ idx }} (流水号)</th>
            </template>
        ...
        <tbody>
          <tr v-if="shipmentsWithGroupTotals.length === 0 && !isLoading && !fetchError">
            <td :colspan="10 + max_production_slots + 1" style="text-align: center;">暂无流向数据或无匹配结果。</td>
          </tr>
          <template v-for="item in shipmentsWithGroupTotals" :key="item.id">
            <tr v-if="!item.isGroupTotalRow" class="shipment-row">
            ...
            <td class="line-column">{{ item.packaging_line }}</td>
              <template v-for="idx in max_production_slots" :key="'data-slot-' + item.id + '-' + idx">
                <td class="dynamic-slot-time-column">
                  {{ formatSlot(item.production_time_slots && item.production_time_slots[idx-1]) }}
                </td>
              </template>
...
export default {
  ...
  computed: {
    ...
    filteredShipments() {
      ...
      const { outer_box_date, material_filter, packaging_line } = this.filters;
  ...
  methods: {
    ...
    async fetchShipments() {
      ...
      try {
        ...
        this.shipments = Array.isArray(data) ? data : (data.results || []);
        this.totalShipmentCount = Array.isArray(data) ? data.length : (data.count || 0);
      } catch (error) {
    ...
    async fetchCustomerByAreaCode() {
      ...
      try {
        ...
        if (response.data && response.data.id) {
          ...
          this.customerFetchError = '未找到该区域码对应的客户。请先在客户信息系统中添加。';
        }
      } catch (error) {
        console.error('查询客户信息失败:', error);
        this.customerFetchError = '查询客户信息失败或未找到客户。';
    ...
    async addShipment() {
      ...
      if (!this.newShipment.customer) {
        this.addError = "客户信息未找到或未选择，请填写并确认区域码。";
        this.isSubmitting = false;
        return;
      }
      ...
      const shipmentPayload = {
        customer_id: this.newShipment.customer, // Renamed 'customer' to 'customer_id'
      ...
      try {
        ...
        if (this.editingShipmentId) {
          response = await axios.put(`${API_URL}/shipments/${this.editingShipmentId}/`, shipmentPayload);
        } else {
          response = await axios.post(`${API_URL}/shipments/`, shipmentPayload);
        }

        // 用轻量提示替代 alert，不阻塞操作
        this.successToast = this.editingShipmentId ? '流向记录已成功修改！' : '流向记录已成功添加！';
        setTimeout(() => { this.successToast = null; }, 2000);

        // 将返回的记录直接更新到本地列表，避免全量刷新
        if (response.data && response.data.id) {
          const idx = this.shipments.findIndex(s => s.id === response.data.id);
          if (idx >= 0) {
            // 更新已有记录
            this.shipments.splice(idx, 1, response.data);
          } else {
            // 新记录插入到列表头部
        ...
        // 不再立即全量刷新，改由轮询或手动刷新时同步
        this.$emit('shipment-data-updated');

      } catch(error) {
        console.error('添加/更新流向记录失败:', error.response || error);
        // Ensure addError is reset or set appropriately even if other alerts are shown
        this.addError = null; // Clear previous technical error before setting a new one
        ...
        if (error.response && error.response.data) {
            if (typeof error.response.data === 'string') {
                ...
                // Try to format a more helpful error from a dict response
                let messages = [];
  ...
  mounted() {
    ...
    this.pollingId = setInterval(this.fetchShipments, 60000);
  },
  beforeUnmount() {
...
/* Styles are assumed to be mostly correct from the previous version that included fixed scrollbar CSS */
/* General Styles */
.shipment-tracking {
...
.modal-error { margin-top: 5px; margin-bottom: 10px; text-align: left; font-size: 0.9em; }
