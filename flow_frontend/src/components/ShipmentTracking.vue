    ...
    <div v-if="isAddShipmentModalVisible" class="modal-overlay">
      <div class="modal modal-lg">
        ...
        <div class="modal-body">
          <form @submit.prevent="handleAddShipmentSubmit" class="shipment-form-in-modal" novalidate>
            <div class="form-grid">
              <div>
              ...
              <div>
                ...
                <input type="text" id="modal-shipment-customer-name" v-model="newShipment.customer_name" :readonly="!!newShipment.customer" placeholder="区域码未注册时可选填" />
              </div>
              <div>
                <label for="modal-shipment-shipping-address">送货地址:</label>
                <textarea id="modal-shipment-shipping-address" v-model="newShipment.shipping_address" :readonly="!!newShipment.customer" placeholder="区域码未注册时可选填"></textarea>
              </div>
              <div>
      ...
      <table ref="actualTable">
        ...
        <tbody>
          ...
          <template v-for="item in shipmentsWithGroupTotals" :key="item.id">
            <tr v-if="!item.isGroupTotalRow" class="shipment-row" :data-shipment-id="item.id" :class="{ 'highlight-new': lastAddedShipmentId === item.id }">
...
export default {
  ...
  data() {
    ...
    return {
      ...
      lastAddedShipmentId: null,  // 用于高亮新添加的记录
      existingRecordIdForAppend: null,  // 409 时存储已有记录 ID，用于追加时间段
      shipmentListVersion: 0,
      shipmentFetchRequestSeq: 0,
      pendingShipmentsRefresh: false,
    };
  },
  computed: {
    availableYears() {
    ...
    shipmentsWithGroupTotals() {
      ...
      // 按录入顺序（ID）排序——升序，最新录入的排在最后
      processedShipments.sort((a, b) => {
        const idA = parseInt(a.original_id || a.id, 10);
        const idB = parseInt(b.original_id || b.id, 10);
        return idA - idB;  // 升序：ID小的（先录入的）在前
      });
      ...
      // 无时间筛选时：按6字段分组，每个原始记录各自创建行
      const grouped = {};
      processedShipments.forEach(originalShipment => {
        if (!originalShipment.customer_details) return;

        const dateForKey = originalShipment.shipment_date || originalShipment.outer_box_date;
        // 分组键：车次只按日期+区域码+品项（4字段），不管规格/线别/批号/外箱日期
        const groupKey = [
          dateForKey,
          originalShipment.customer_details.area_code,
          originalShipment.material_number,
        ...
        const allSlots = originalShipment.production_time_slots || [];
  ...
  methods: {
    ...
    getShipmentDisplayYear(shipment) {
      const dateString = shipment && (shipment.shipment_date || shipment.outer_box_date);
      if (!dateString) return null;
      const year = new Date(dateString).getFullYear();
      return Number.isNaN(year) ? null : year;
    },
    shouldShowShipmentForSelectedYear(shipment) {
      if (!this.selectedYear) return true;
      return this.getShipmentDisplayYear(shipment) === Number(this.selectedYear);
    },
    sortShipmentsForDisplay() {
      this.shipments.sort((a, b) => {
        const dateCompare = String(b.outer_box_date || '').localeCompare(String(a.outer_box_date || ''));
        if (dateCompare !== 0) return dateCompare;
        return Number(b.id || 0) - Number(a.id || 0);
      });
    },
    upsertShipmentFromResponse(shipment) {
      if (!shipment || !shipment.id) return;
      this.shipmentListVersion += 1;
      const shipmentId = Number(shipment.id);
      this.shipments = this.shipments.filter(item => Number(item.id) !== shipmentId);
      if (this.shouldShowShipmentForSelectedYear(shipment)) {
        this.shipments.unshift(shipment);
        this.sortShipmentsForDisplay();
      }
    },
    queueShipmentsRefresh() {
      if (this.pendingShipmentsRefresh) return;
      this.pendingShipmentsRefresh = true;
      ...
      const run = () => {
        ...
        this.pendingShipmentsRefresh = false;
        this.fetchShipments({ silent: true });
      };

      setTimeout(run, 0);
    },
    focusShipmentRow(shipmentId) {
      if (!shipmentId) return;
      const targetRowId = `${shipmentId}-row-0`;
      this.lastAddedShipmentId = targetRowId;
      this.$nextTick(() => {
        const row = document.querySelector(`tr[data-shipment-id="${targetRowId}"]`);
        if (row) {
          row.scrollIntoView({ behavior: 'smooth', block: 'center' });
          setTimeout(() => {
            if (this.lastAddedShipmentId === targetRowId) {
              this.lastAddedShipmentId = null;
            }
          }, 3000);
        }
      });
    ...
    async fetchShipments(options = {}) {
      if (this.isLoading) {
        console.log("Polling skipped: a fetch is already in progress.");
        return;
      }
      const silent = options && options.silent === true;
      const requestSeq = ++this.shipmentFetchRequestSeq;
      const listVersionAtStart = this.shipmentListVersion;
      this.isLoading = true;
      if (!silent) {
      ...
      try {
        ...
        const nextShipments = response.data.results !== undefined ? response.data.results : response.data;
        if (requestSeq === this.shipmentFetchRequestSeq && listVersionAtStart === this.shipmentListVersion) {
          this.shipments = nextShipments;
        }
      } catch (error) {
    ...
    async fetchCustomerByAreaCode() {
      ...
      try {
        ...
        if (response.data && response.data.id) {
          ...
          // 客户不存在，字段可手动输入，后端会自动创建只含区域码的客户
          this.customerFetchError = '区域码未注册，将自动创建新客户。客户名称和地址可选填。';
        }
      } catch (error) {
        // 404 或其他错误，均视为客户不存在，允许继续提交
        this.customerFetchError = '区域码未注册，将自动创建新客户。客户名称和地址可选填。';
      } finally {
        this.isCustomerFetching = false;
      }
    ...
    async addShipment() {
      ...
      try {
        ...
        this.upsertShipmentFromResponse(apiResponse.data);
        this.queueShipmentsRefresh();

        // 定位并高亮新记录（display row ID 格式：${recordId}-row-0）
        if (newShipmentId) {
            this.focusShipmentRow(newShipmentId);
            // 显示非阻塞提示
            this.showSuccessToast(`流向记录已添加（ID: ${newShipmentId}），已滚动到列表中对应位置。`);
        } else {
    ...
    async appendTimeSlots() {
      ...
      try {
        const appendedShipmentId = this.existingRecordIdForAppend;
        ...
        // 追加成功后：清空时间段（保留其他字段）、关闭模态框、刷新列表
        this.resetFormForNextEntry();
        this.upsertShipmentFromResponse(resp.data.shipment || resp.data);
        this.queueShipmentsRefresh();

        // 高亮被追加的记录（display row 的 id 是 ${recordId}-row-0）
        this.focusShipmentRow(appendedShipmentId);

        this.showSuccessToast(`${msg}，已滚动到记录位置。`);
    ...
    formatOuterBoxDateForDisplay(dateString) {
      ...
      const prefix = (year > 2026 || (year === 2026 && month >= 5)) ? 'YC' : 'YY';
